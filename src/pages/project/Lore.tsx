...
export default function LorePage() {
  ...
  return <div className="flex h-[calc(100vh-4rem)] animate-fade-in">
      ...
      <div className="w-80 border-r border-border flex flex-col">
        ...
        <div className="p-4 border-b border-border">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-1">
              <AIGenerateButton type="lore" context={activeCategory !== 'all' ? `Create a ${CATEGORY_CONFIG[activeCategory].label} entry for my story world.` : ''} onGenerated={content => {
        ...
        setSelectedEntry(null);
          ...
          <TabsList className="flex flex-wrap gap-1 p-2 h-auto bg-transparent justify-start">
            ...
            return <TabsTrigger key={key} value={key} className={cn("text-xs px-2 py-1 gap-1", `data-[state=active]:${config.color}`)}>
          ...
          <div className="flex-1 overflow-y-auto p-3">
            <div className="space-y-2">
              ...
              const Icon = config.icon;
              return <button key={entry.id} onClick={() => setSelectedEntry(entry.id)} className={cn("w-full text-left p-3 rounded-lg transition-all duration-200 border", config.color, selectedEntry === entry.id ? "ring-2 ring-primary ring-offset-1 ring-offset-background" : "hover:opacity-80")}>
      ...
      <div className="flex-1 p-6 overflow-y-auto">
        {selected ? <div className="max-w-3xl mx-auto animate-fade-in">
            <div className="flex items-start justify-between mb-6">
              <div className="flex-1">
                <Input value={selected.title} onChange={e => updateLoreEntry(selected.id, {
              title: e.target.value
                ...
                return <button key={key} onClick={() => updateLoreEntry(selected.id, {
                  category: key as LoreEntry['category']
                })} className={cn("flex items-center gap-1 px-2 py-1 rounded-md text-xs border transition-all", isActive ? config.color : "border-border text-muted-foreground hover:border-primary/30")}>
                        <Icon className="h-3 w-3" />
                        {config.label}
                      </button>;
              })}
            ...
            <Card className={cn("p-3 mb-6 border", CATEGORY_CONFIG[selected.category as Category]?.color || CATEGORY_CONFIG.general.color)}>
            ...
            <div className="relative">
              <Textarea value={selected.content || ''} onChange={e => updateLoreEntry(selected.id, {
            content: e.target.value
          })} placeholder="Describe this aspect of your world in detail..." className="min-h-[400px] bg-secondary/30 font-display text-lg leading-relaxed resize-none" />
              <div className="absolute top-2 right-2">
                <AIGenerateButton type="lore" context={`Expand on this ${CATEGORY_CONFIG[selected.category as Category]?.label || 'lore'} entry titled "${selected.title}": ${selected.content || ''}`} onGenerated={content => {
              updateLoreEntry(selected.id, {
            ...
            <div className="mt-4">
              ...
              <Input value={selected.tags?.join(', ') || ''} onChange={e => updateLoreEntry(selected.id, {
            tags: e.target.value.split(',').map(t => t.trim()).filter(Boolean)
          })} placeholder="Add tags separated by commas..." className="bg-secondary/30" />
            </div>
