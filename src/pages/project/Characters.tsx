  ...
  return <div className="flex h-[calc(100vh-4rem)] animate-fade-in">
      ...
      <div className="w-80 border-r border-border p-4 overflow-y-auto">
        ...
        <div className="space-y-2">
          {characters.map(character => <button key={character.id} onClick={() => setSelectedCharacter(character.id)} className={`w-full text-left p-3 rounded-lg transition-all ${selectedCharacter === character.id ? 'bg-primary/10 border border-primary/30' : 'bg-card border border-border hover:border-primary/20'}`}>
      ...
      <div className="flex-1 p-6 overflow-y-auto">
        {selected ? <div className="max-w-3xl mx-auto">
            <div className="flex items-start justify-between mb-6">
              <div className="flex-1">
                <Input value={selected.name} onChange={e => updateCharacter(selected.id, {
              name: e.target.value
            })} className="text-2xl font-display font-bold bg-transparent border-transparent hover:border-border h-auto py-1" placeholder="Character name..." />
              </div>
              <div className="flex items-center gap-2">
                <select value={selected.role} onChange={e => updateCharacter(selected.id, {
              role: e.target.value as Character['role']
                ...
                <Button variant="ghost" size="icon" className="text-muted-foreground hover:text-destructive" onClick={() => handleDeleteCharacter(selected.id)}>
